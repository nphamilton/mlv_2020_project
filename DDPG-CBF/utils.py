...
def LINEAR(x, hidden, name='None'):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        weight = tf.get_variable('Weight', [x.get_shape()[-1], hidden],
                                 initializer=tf.truncated_normal_initializer(stddev=0.01))  # Near empty NN
...
def FLAT_GRAD(loss, vrbs):
    ...
    return tf.concat(values=[tf.reshape(g, [np.prod(v.get_shape().as_list()), ]) for (g, v) in zip(grads, vrbs)],
                     axis=0)
...
class GetValue:
    def __init__(self, sess, variable_list, name=None):
        self.name = name
        self.sess = sess
        self.op_list = tf.concat(axis=0,
                                 values=[tf.reshape(v, [np.prod(v.get_shape().as_list())]) for v in variable_list])
